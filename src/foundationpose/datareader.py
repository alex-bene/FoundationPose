...
from foundationpose.Utils import depth2xyzmap, symmetry_tfs_from_info, trimesh_add_pure_colored_texture
...
class BopBaseReader:
    def __init__(self, base_dir, zfar=np.inf, resize=1):
        ...
        self.K_table = {}
        with open(f"{self.base_dir}/scene_camera.json") as ff:
            info = json.load(ff)
        for k in info:
        ...
        if os.path.exists(f"{self.base_dir}/scene_gt.json"):
            with open(f"{self.base_dir}/scene_gt.json") as ff:
    ...
    def make_scene_ob_ids_dict(self):
        with open(f"{BOP_DIR}/{self.dataset_name}/test_targets_bop19.json") as ff:
...
class YcbVideoReader(BopBaseReader):
    def __init__(self, base_dir, zfar=np.inf):
        ...
        if "BOP" not in self.base_dir:
            with open(f"{self.base_dir}/../../keyframe.txt") as ff:
